...
function containsInappropriateContent(text) {
    ...
    // Normalize input text (remove accents, lowercase)
    ...
    return filterWords.some(word => {
        try {
            ...
            // Fallback for older browsers (though Electron should support it)
            // or if the regex is invalid for some reason
            const regex = new RegExp(`(^|[^a-zA-Z0-9\u00C0-\u017F\u0370-\u03FF\u0400-\u04FF])` + word + `([^a-zA-Z0-9\u00C0-\u017F\u0370-\u03FF\u0400-\u04FF]|$)`, 'i');
            return regex.test(normalizedInput);
        }
    });
}
