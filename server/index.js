...
app.get('/api/discovery-info', (req, res) => {
  ...
  res.json({
    ...
    version: '10.2.1-beta', // Should match package.json
    classes: classes
  });
});
...
async function trainClassifier() {
  try {
    ...
    if (!modelLoaded) {
      ...
      let trainingData = { blacklist: [], whitelist: [], clean: [], profane: [] };
      ...
      if (fs.existsSync(specificDataPath)) {
        ...
        trainingData = { ...trainingData, ...JSON.parse(raw) };
      } else {
        // Fallback to internal data if specific file missing
        console.log('🧠 Specific data file not found, looking for internal defaults...');
        const internalDataPath = path.join(__dirname, 'public', 'training-data.json');
        if (fs.existsSync(internalDataPath)) {
          trainingData = { ...trainingData, ...JSON.parse(fs.readFileSync(internalDataPath, 'utf-8')) };
        }
      }

      // Train Blacklist / Profane
      ...
      if (trainingData.profane && Array.isArray(trainingData.profane)) {
        console.log(`🧠 Training ${trainingData.profane.length} profane items...`);
        for (const word of trainingData.profane) {
      ...
      if (trainingData.clean && Array.isArray(trainingData.clean)) {
        console.log(`🧠 Training ${trainingData.clean.length} clean items...`);
        for (const word of trainingData.clean) {
          if (typeof word === 'string') {
      ...
      // 3.5 Train with mild good words to ensure 'clean' category exists
      // and has a realistic baseline
...
io.on('connection', (socket) => {
  ...
  socket.on('join-or-create-lobby', ({ userName }, callback) => {
    ...
    if (typeof callback === 'function') callback({
      ...
      blockAllActive: classData.blockAllActive || false, // New
      blockUploadsActive: classData.blockUploadsActive || false, // New
      allowHandsUp: classData.allowHandsUp !== undefined ? classData.allowHandsUp : true, // New
      autoDownloadEnabled: classData.autoDownloadEnabled || false, // New
      autoDownloadPath: classData.autoDownloadPath || '' // New
    });
  ...
  socket.on('take-over-lobby', ({ userName }, callback) => {
    ...
    if (!activeClasses.has(lobbyId)) {
      ...
      if (typeof callback === 'function') callback({
        ...
        blockAllActive: false, // New
        blockUploadsActive: false, // New
        allowHandsUp: true, // New
        autoDownloadEnabled: false, // New
        autoDownloadPath: '' // New
      });
      return;
  ...
  socket.on('join-class', ({ classId, userName }, callback) => {
    ...
    // Determine if user is blocked and why
    let blockedReason = null;
    if (classData.blockedUsers && classData.blockedUsers.has(socket.id)) {
      blockedReason = 'teacher-blocked';
    } else if (classData.blockAllActive && role === 'student') {
  ...
  // Request system path
  socket.on('request-system-path', ({ folderType, classId }, callback) => {
    try {
      if (!activeClasses.has(classId)) {
        if (typeof callback === 'function') callback({ success: false, message: 'Class not found' });
        return;
      ...
      switch (folderType) {
        case 'desktop':
          systemPath = path.join(homeDir, 'Desktop');
          break;
        case 'documents':
          systemPath = path.join(homeDir, 'Documents');
          break;
        case 'downloads':
          systemPath = path.join(homeDir, 'Downloads');
          break;
        default:
          systemPath = homeDir;
      }
      ...
      if (typeof callback === 'function') {
        callback({ success: true, path: systemPath });
      }
    } catch (error) {
      console.error('Error resolving system path:', error);
      if (typeof callback === 'function') {
        callback({ success: false, message: 'Failed to resolve path' });
      }
    }
  });
  ...
  socket.on('update-advanced-settings', ({ classId, settings }) => {
    ...
    // Sanitize and bound thresholds (10-90% to avoid extreme 0% blocking everything or 100% blocking nothing)
    const blockThreshold = Math.max(10, Math.min(90, settings.blockThreshold !== undefined ? settings.blockThreshold : 50));
    const reportThreshold = Math.max(10, Math.min(90, settings.reportThreshold !== undefined ? settings.reportThreshold : 30));
  ...
  socket.on('request-high-res-frame', ({ targetUserId }) => {
    const classId = getTeacherClassId(socket.id);
    if (classId) {
      // Direct relay to specific student
      const targetSocket = io.sockets.sockets.get(targetUserId);
      if (targetSocket) {
        targetSocket.emit('request-high-res-frame');
      }
    }
  });
  ...
  socket.on('monitoring-frame', ({ frame, userId, userName, isHighRes }) => {
    ...
    if (classId) {
      ...
      if (classData && classData.teacherId) {
        // Find teacher's socket and emit directly or use io.to(teacherId)
...
function getUserClassId(socketId) {
  for (const [classId, data] of activeClasses.entries()) {
    if (data.users && data.users.some(u => u.id === socketId)) {
      return classId;
    }
  }
  return null;
